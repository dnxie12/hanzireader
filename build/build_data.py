...
def generate_learn_order(chars):
    ...
    common_radicals = ['一', '人', '大', '口', '女', '子', '小', '山', '木', '水',
                        '火', '土', '日', '月', '金', '手', '心', '目', '刀', '力',
                        '门', '马', '王', '石', '田']
    ...
    print(f"  Learn order: {len(order)} characters ({len(phase1)} phase 1, {len(phase23)} phase 2-3, {len(phase4)} phase 4)")
    return order
...
def main():
    ...
    for sample in ['的', '机', '河', '好', '学']:
        if sample in chars:
            ...
            print(f"  {sample} [{info['p']}] #{info['f']} — {info['d'][:40]} — compounds: {cw_str}")
